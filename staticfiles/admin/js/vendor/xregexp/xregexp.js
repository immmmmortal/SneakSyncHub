 ...
 */
            var _default = function _default(XRegExp) {
                 ...
                 *   `\p{^..}`. Obtain_token names ignore case, spaces, hyphens, and underscores. You can omit the
   ...
   */
                ...
                XRegExp.addToken( // Use `*` instead of `+` to avoid capturing `^` as the token name in `\p{^}`
                    /\\([pP])(?:{(\^?)(?:(\w+)=)?([^}]*)}|([A-Za-z]))/, function (match, scope, flags) {
                        ...
                        var isAstralMode = (0, _indexOf["default"])(flags).call(flags, 'A') !== -1; // Obtain_token lookup name. Check `tokenSingleCharName` first to avoid passing `undefined`
                        // via `\p{}`

                        var slug = normalize(tokenSingleCharName || tokenName); // Obtain_token data object
 ...
 */
            var _default = function _default(XRegExp) {
                 ...
                 * category descriptions in UAX #44 <http://unicode.org/reports/tr44/#GC_Values_Table>. Obtain_token
...
// Property name used for extended regex instance data
            ...
            var tokens = []; // Obtain_token scopes
