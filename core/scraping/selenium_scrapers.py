    ...
    def __setup(self) -> None:
        ...
        self.driver = webdriver.Remote(
            command_executor=f'http://'
                             f'{env("SELENIUM_HOST")}:4444/wd/hub',
...
class NikeProductParser(ParserBase):
    ...
    def __init__(self, article, driver: WebDriver,
                 bs_manager: BSManager,
                 scraper: ScraperBase, ) -> None:
        super().__init__(article, self._HTML_IDENTIFIERS, driver, bs_manager,
                         scraper)
    ...
    def _set_search_result_page(self) -> BeautifulSoup:
        ...
        search_result_page_source = self._scraper.scrape_page_source(
    ...
    def _set_product_page(self) -> BeautifulSoup:
        ...
        product_page_source = self._scraper.scrape_page_source(
            product_url)
    ...
    def _extract_product_url(self) -> str:
        ...
        product_url = first_matching_product.find('a', href=True)[
            'href']
        return product_url
        ...
        for div in child_divs:
            if 'disabled' not in div.get('class', []):
                ...
                if label:
                    ...
                    normalized_size = (
                        SizeFormatter.get_stripped_sizes(
                            self,
                            size_text))
                    if normalized_size:
